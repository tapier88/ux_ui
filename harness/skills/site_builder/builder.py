...
class SiteBuilder:
    ...
    def _handle_navigation(self, design_build_plan: Dict[str, Any]):
        ...
        navigation comes from design_execution_planner's NavigationPlan:
        a list of {label, href, section_id} items generated from the
        sections actually in the plan, plus style/mobile_pattern/sticky/cta
        settings. Writes a Navigation.tsx component listing those links -
        previously there was no navigation field on DesignBuildPlan at all,
        so this had nothing to act on regardless of what this method did.
        """
        navigation = design_build_plan.get("navigation", {})

        if not navigation or not navigation.get("items"):
            return

        items = navigation["items"]
        links_jsx = "\n".join(
            f'      <a href="{item.get("href", "#")}" className="nav-link">{item.get("label", "")}</a>'
            for item in items
        )
        cta_jsx = ""
        if navigation.get("cta_label") and navigation.get("cta_href"):
            cta_jsx = (
                f'\n      <a href="{navigation["cta_href"]}" className="btn-primary">'
                f'{navigation["cta_label"]}</a>'
            )

        sticky_class = " sticky top-0 z-50" if navigation.get("sticky") else ""
        nav_content = f"""import React from 'react';

export const Navigation: React.FC = () => {{
  return (
    <nav className="nav{sticky_class}">
{links_jsx}{cta_jsx}
    </nav>
  );
}};
...
"""
        nav_path = "src/components/Navigation.tsx"
        self.file_manager.create_file(
            nav_path, nav_content, reason="Implement site navigation"
        )
        self.report.files_created.append(nav_path)
    ...
    def _handle_interactions(self, design_build_plan: Dict[str, Any]):
        ...
        interactions is bridged (via DesignPipelineNode's adapter) from
        design_execution_planner's motion_plan: List[MotionPlan] - each
        entry a real animation definition (target, trigger, type, duration,
        easing, etc), generated by MotionPlanner. Writes them as a
        documented reference file (framer-motion prop values per animated
        target) rather than trying to inject props into arbitrary already
        -generated component files without knowing their exact JSX
        structure.
        """
        ...
        lines = ["# Motion / interactions plan (from design_execution_planner)", ""]
        for motion in interactions:
            if not isinstance(motion, dict):
                continue
            lines.append(f"## {motion.get('target', 'unknown target')}")
            lines.append(f"- Trigger: {motion.get('trigger', 'load')}")
            lines.append(f"- Type: {motion.get('type', 'fade')}")
            lines.append(f"- Duration: {motion.get('duration', '300ms')}")
            lines.append(f"- Easing: {motion.get('easing', 'ease')}")
            if motion.get("reduced_motion_behavior"):
                lines.append(f"- Reduced motion: {motion['reduced_motion_behavior']}")
            lines.append("")

        interactions_path = "MOTION_PLAN.md"
        self.file_manager.create_file(
            interactions_path, "\n".join(lines), reason="Document motion/interactions plan"
        )
